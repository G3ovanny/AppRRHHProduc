# Generated by Django 4.2.2 on 2023-11-09 17:26

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import simple_history.models


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('trabajadores', '0010_remove_datospersonalestrabajadores_nombre_enfermedad_and_more'),
    ]

    operations = [
        migrations.CreateModel(
            name='InformacionBancaria',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('state', models.BooleanField(default=True, verbose_name='Estado')),
                ('created_date', models.DateTimeField(auto_now_add=True, null=True, verbose_name='Fecha de creación')),
                ('modified_date', models.DateTimeField(auto_now=True, null=True, verbose_name='Fecha de modificación')),
                ('delete_date', models.DateTimeField(auto_now=True, null=True, verbose_name='Fecha de eliminación')),
                ('institucion_bancaria', models.CharField(blank=True, max_length=255, null=True, verbose_name='Institución financiera')),
                ('tipo_cuenta', models.CharField(blank=True, choices=[('Ahorros', 'AHORROS'), ('Corriente', 'CORRIENTE')], max_length=255, null=True, verbose_name='Tipo cuenta bancaria')),
                ('num_cuenta', models.CharField(blank=True, max_length=255, null=True, verbose_name='Número cuenta bancaria')),
            ],
            options={
                'verbose_name': 'Información Bancaria',
                'verbose_name_plural': 'Informacion Bancarias',
                'db_table': 'Informacion_bancaria',
                'ordering': ['id'],
            },
        ),
        migrations.AlterField(
            model_name='datospersonalestrabajadores',
            name='tipo_pasaporte',
            field=models.CharField(blank=True, choices=[('Pasaporte', 'PASAPORTE'), ('Visa', 'VISA'), ('Carnet de refugiado', 'CARNET DE REFUGIADO')], max_length=255, null=True, verbose_name='Pasaporte'),
        ),
        migrations.AlterField(
            model_name='historicaldatospersonalestrabajadores',
            name='tipo_pasaporte',
            field=models.CharField(blank=True, choices=[('Pasaporte', 'PASAPORTE'), ('Visa', 'VISA'), ('Carnet de refugiado', 'CARNET DE REFUGIADO')], max_length=255, null=True, verbose_name='Pasaporte'),
        ),
        migrations.CreateModel(
            name='HistoricalInformacionBancaria',
            fields=[
                ('id', models.IntegerField(blank=True, db_index=True)),
                ('state', models.BooleanField(default=True, verbose_name='Estado')),
                ('created_date', models.DateTimeField(blank=True, editable=False, null=True, verbose_name='Fecha de creación')),
                ('modified_date', models.DateTimeField(blank=True, editable=False, null=True, verbose_name='Fecha de modificación')),
                ('delete_date', models.DateTimeField(blank=True, editable=False, null=True, verbose_name='Fecha de eliminación')),
                ('institucion_bancaria', models.CharField(blank=True, max_length=255, null=True, verbose_name='Institución financiera')),
                ('tipo_cuenta', models.CharField(blank=True, choices=[('Ahorros', 'AHORROS'), ('Corriente', 'CORRIENTE')], max_length=255, null=True, verbose_name='Tipo cuenta bancaria')),
                ('num_cuenta', models.CharField(blank=True, max_length=255, null=True, verbose_name='Número cuenta bancaria')),
                ('history_id', models.AutoField(primary_key=True, serialize=False)),
                ('history_date', models.DateTimeField(db_index=True)),
                ('history_change_reason', models.CharField(max_length=100, null=True)),
                ('history_type', models.CharField(choices=[('+', 'Created'), ('~', 'Changed'), ('-', 'Deleted')], max_length=1)),
                ('history_user', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='+', to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'historical Información Bancaria',
                'verbose_name_plural': 'historical Informacion Bancarias',
                'ordering': ('-history_date', '-history_id'),
                'get_latest_by': ('history_date', 'history_id'),
            },
            bases=(simple_history.models.HistoricalChanges, models.Model),
        ),
    ]
